...
# file directory
d="/content/drive/MyDrive/Group 1 Mini project 2022-23: New Dataset /Sarthak"
l1=listdir(d)
Files=[]
for l in l1:
    if(l.endswith("xml")):
...
def process(Files,columns,metadata):
    for file in Files:
        ...
        for i in range(len(df['name'])):
            df5.loc[len(df5.index)+1]=[str('\\content\\drive\\MyDrive\\Group 1 Mini project 2022-23: New Dataset \\Sarthak'+metadata['filename'][0]),str(int(df['xmin'][i])),str(int(df['ymin'][i])),str(int(df['xmax'][i])),str(int(df['ymax'][i])),str(df['name'][i])]
